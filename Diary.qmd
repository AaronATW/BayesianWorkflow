...
-   **Model**: $$\Delta y_{it}=\beta_0 + \beta_1*time + \beta_2*mdur_i+ \beta_3*mbase_i + b_i + \epsilon_{it}$$ $$b_i\sim\bf{N}(0,\sigma_b^2),\quad\epsilon\sim\bf{N}(0,\sigma^2)$$
...
-   For intercept $\beta_0$, I actually have no ideas what it would be like. So I would choose $\mathbf{N}(0, 10^2)$ as the prior.

-   Priors of the error term still keep this form: $\quad\sigma\sim\mathbf{C}(0,2)$
...
hist(prior_samples$sd, main = "sd", xlab = "sd", breaks = 100)
```
