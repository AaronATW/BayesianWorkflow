...
From the LOO comparison result, we used the prediction method to select the best model we provided. This form shows that the Gaussian GAM model has a significant better prediction ability than the other two. Therefore, we chose this model as our final prediction model.
...
ggplot(all_results, aes(x = time, y = mean, color = combination)) +
  ...
  labs(title = "Predicted Values with 50% Credible Intervals for Different mdur Values (mbase=50)",
       x = "Time",
       y = "Predicted yi") +
  theme_minimal() +
  scale_color_discrete(name = "mdur Combination")
...
ggplot(all_results_mbase, aes(x = time, y = mean, color = combination)) +
  ...
  geom_line(aes(y = lower), linetype = "dashed", size = 0.5) +
  geom_line(aes(y = upper), linetype = "dashed", size = 0.5) +
  labs(title = "Predicted Values with 50% Credible Intervals for Different mbase Values (mdur = 15)",
       x = "Time",
       y = "Predicted yi") +
  theme_minimal() +
  scale_color_discrete(name = "mbase Combination")
```
...
-   Mdur has a vague impact on the trend of the treatment effect, because we can see from the plot that confidence intervals are overlapping and there is no clear trend which direction these curves go.

-   In contrast, mbase has a clearer impact on the treatment effect. Higher mbase values, which implies more severe disease, corresponds to stronger observed mean differences. This shows that DBS treatment has a stronger effect dealing with worse functioning patients with higher UPDRS scores.
